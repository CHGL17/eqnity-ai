...
def _find_fx(track, vst_name):
    for fx in track.fxs:
        # A veces el nombre incluye 'VST: ' o 'VST3: '. Lo ignoramos.
        clean_fx_name = fx.name.split(': ')[-1]
        if clean_fx_name.lower() == vst_name.lower():
            return fx, None
...
# Esto le da al LLM una estructura clara que debe seguir.
...
def list_tracks_and_vsts() -> str:
    ...
    try:
        ...
        track_info = [
            f"Pista: '{track.name}' | VSTs: {[fx.name for fx in track.fxs]}"
...
def set_multiple_vst_parameters(track_name: str, vst_name: str, changes: List[ParameterChange]) -> str:
    ...
    try:
        ...
        if error or fx is None:
            return error if error else f"Error: No se pudo encontrar el VST '{vst_name}' en la pista '{track_name}'."
            ...
            for p in fx.params:
                if p.name.lower() == change.parameter_name.lower():
                    if 0.0 <= change.value <= 1.0:
                        p = change.value
                        results.append(f"  - '{p}' ajustado a {change.value:.2f}.")
                        param_found = True
                    else:
...
tools = [list_tracks_and_vsts, list_vst_parameters, set_multiple_vst_parameters]
...
# (SOLUCIÓN PUNTO 4) Prompt mejorado para ser más proactivo y usar las nuevas herramientas.
prompt_template = """
<role>
...
# (SOLUCIÓN PUNTO 3) Configuración de la memoria
...
def main():
    ...
    # ... (código de conexión igual que antes)

    while True:
        user_query = input("\nUsuario > ")
        if user_query.lower() in ["salir", "exit", "quit"]: break
